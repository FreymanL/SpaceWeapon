...
const TARGET_DESTROYED_ENEMIES = 90
...
func _ready():
	world_info = game_settings.get_key("worlds")["world_5"]
...
func spawn_snow_cruise(num: int):
	for i in num:
		var spawned = snow_cruise_enemy.instantiate()
		var rng = RandomNumberGenerator.new()
